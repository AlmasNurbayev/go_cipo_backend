...
# TODO
- [ ] добавить ручки для получения result "истории истории" - для каждого responseID из kaspi_export_goods_registry - с параметром "i", получить результат проверки. 
Принимать с фронта responseID, делать запрос и без парсинга/изменений возвращать как any на Фронт.
Пример: api/products/import/result?i=076f1ba2-7728-4468-8c46-51656ddbd8de
- [ ] в таблицу store добавить код склада kaspi 
...
- [v] новый парсер для самописной выгрузки JSON из 1С Розница 2.3 - учесть:
    1. не совпадают ГУИД по size/char - типовой обмен использовал доп. сведения вместо характеристик
    2. нет некоторых обязательных атрибутов регистратора/каталога
    3. в новой сампописной выгрузке есть ШК, нужно записывать в qnt_price_registry для возможного сопоставления с kaspi
    4. товарная группа теперь выгружается из номенклатуры, а не доп. сведений, проверить соответствие гуидов
    5. Вид номенклатуры, дублируются гуиды - но не используются в таблицах
    6. доп сведения ("материал подошвы") теперь выгружаются с добавлением товарной группы в скобках ("материал подошвы (обувь детская)"), проверить что сопоставляется по гуидам
    7. в новой выгрузке добавлена Марка по гуиду и наименованию, возможно нужна таблица или записывать как строку
