...
func (s *Service) KaspiUpdateCategory(ctx context.Context, request dto.KaspiUpdateCategoryRequest) (dto.KaspiUpdateCategoryResponse, error) {
	op := "services.KaspiUpdateCategory"
	log := s.log.With(slog.String("op", op))

	var newCategory models.KaspiCategoriesEntity
	var response dto.KaspiUpdateCategoryResponse
	...
	oldCategory, err := s.kaspiStorage.GetByIdKaspiCategory(ctx, request.Id)
	if err != nil {
		return response, err
	}
	log.Debug("oldCategory", slog.String("oldCategory name", oldCategory.Name_kaspi))
	newCategory.Id = oldCategory.Id
	newCategory.Name_kaspi = oldCategory.Name_kaspi
	...
	titleIndex := slices.IndexFunc(categories, func(category category) bool {
		return category.code == oldCategory.Name_kaspi
	})
	if titleIndex == -1 {
		return response, errors.New("category code not found")
	} else {
	...
	//получаем список обязательных атрибутов
	status, body, err := clients.KaspiGetAttributes(s.cfg, s.log, oldCategory.Name_kaspi, token)
	if err != nil {
		return response, err
	}
	var attributes []map[string]any
	...
	values := clients.KaspiGetAllValues(s.cfg, s.log, oldCategory.Name_kaspi, token)
	// strValues, _ := utils.PrintAsJSON(values)
	// fmt.Println("strValues", string(*strValues))
	for _, attribute := range values {
		slices.Sort(attribute.Items)
		...
		case "Shoes*Model":
			newCategory.Model_kaspi = attribute.Items
		default:
			log.Warn("unknown attribute", slog.String("attribute_name", attribute.ValueName))
		}
	}

	// обновляем категорию в базе
	err = s.kaspiStorage.UpdateKaspiCategory(ctx, newCategory)
	if err != nil {
		log.Error("", slog.String("err", err.Error()))
		return response, err
	}

	err = copier.Copy(&response, &newCategory)
	if err != nil {
		log.Error("", slog.String("err", err.Error()))
		return response, err
	}
