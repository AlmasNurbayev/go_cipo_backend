...
func (s *Storage) ListKaspiProductsSearch(ctx context.Context, registrator_id int64, params dto.KaspiProductsQueryRequest) ([]models.ProductsItemEntity, int, error) {
	...
	qntDistinct := sb.Select(
		...
		Where(squirrel.Eq{"qpr.registrator_id": registrator_id, "p.kaspi_in_sale": true}).
		Distinct()
